...
Endpoints:
  GET  /api/health
  POST /api/analyze/upload        — multipart audio file (single model)
  POST /api/analyze/record        — raw recorded blob (single model)
  POST /api/analyze/compare       — multipart audio, runs all 3 models + severity
  POST /api/analyze/compare/record — recorded blob, runs all 3 models + severity
"""
...
from predict import analyze, analyze_all, MODELS
...
def analyze_compare():
    ...
    tmp_path = _save_temp(file, suffix)
    try:
        result = analyze_all(tmp_path, filename=file.filename)
        return jsonify(result)
    except Exception as e:
        app.logger.exception('Compare analysis error')
        return jsonify({'error': f'Analysis failed: {e}'}), 500
    finally:
        try: os.remove(tmp_path)
        except OSError: pass
...
@app.post('/api/analyze/compare/record')
def analyze_compare_record():
    if 'audio' not in request.files:
        return jsonify({'error': 'No audio blob provided'}), 400
    file     = request.files['audio']
    tmp_path = _save_temp(file, '.webm')
    try:
        result = analyze_all(tmp_path, filename='recording.webm')
        return jsonify(result)
    except Exception as e:
        app.logger.exception('Compare record error')
        return jsonify({'error': f'Analysis failed: {e}'}), 500
    finally:
        try: os.remove(tmp_path)
        except OSError: pass
